// components/HospitalsWithMapTailwind.tsx
'use client';
  ...
  const html = `
    <svg width="44" height="56" viewBox="0 0 44 56" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="22" y="26" text-anchor="middle" font-size="10" font-weight="700" fill="#1e3a8a"
        font-family="system-ui,-apple-system, Segoe UI, Roboto, Arial">${txt.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</text>
    </svg>`;
...
export default function HospitalsWithMapTailwind({
  ...
  const filtered = useMemo(() => {
    ...
    return rowsWithLoc.filter(r =>
      `${r.hospital_name} ${r.contact_person} ${r.license_no} ${r.phone} ${r.city} ${normState(r.state)}`
        .toLowerCase()
        .includes(s)
    );
  }, [rowsWithLoc, q]);
      ...
      for (const h of filtered) {
        ...
        try {
          ...
          if (!markersRef.current.get(h.user_id)) {
            const m = L.marker([lat, lng], { icon: hospitalIcon(h.hospital_name) }).addTo(mapRef.current);
            m.bindPopup(
            ...
            markersRef.current.get(h.user_id)!.setLatLng([lat, lng]).setIcon(hospitalIcon(h.hospital_name));
          }
          bounds.push([lat, lng]);
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-slate-600 truncate mt-0.5">{h.city}, {normState(h.state)}</div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 shrink-0">
                        ...
                        <button
                          ...
                          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-300 bg-white text-xs px-3 py-1.5
                                     hover:bg-slate-50 active:scale-[0.98] transition"
                          title="Call"
                        >
                        ...
                        <a
                          href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${h.hospital_name}, ${h.city}, ${normState(h.state)}`)}`}
                          target="_blank" rel="noreferrer"
                          className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-gradient-to-br from-blue-600 to-indigo-600 text-white text-xs px-3 py-1.5
                                     shadow-sm hover:from-blue-700 hover:to-indigo-700 active:scale-[0.98] transition"
